...
import com.winlator.compat.ChromiumCompatHelper;
import com.winlator.compat.CrashAnalyzer;
...
public class GuestProgramLauncherComponent extends EnvironmentComponent {
    ...
    /** Accumulated log lines for crash analysis. Flushed on process exit. */
    private final CrashAnalyzer.CrashReport sessionCrashReport = new CrashAnalyzer.CrashReport();
    /** Optional callback invoked when a critical crash pattern is detected. */
    private Callback<CrashAnalyzer.CrashReport> crashReportCallback;
    ...
    public Callback<CrashAnalyzer.CrashReport> getCrashReportCallback() {
        return crashReportCallback;
    }

    /** Set a callback that will be invoked whenever a CRITICAL crash finding is detected in logs. */
    public void setCrashReportCallback(Callback<CrashAnalyzer.CrashReport> crashReportCallback) {
        this.crashReportCallback = crashReportCallback;
    }

    /** Returns the accumulated crash report for this session. */
    public CrashAnalyzer.CrashReport getSessionCrashReport() {
        return sessionCrashReport;
    }
    ...
    private int execGuestProgram() {
        ...
        // ── Phase 3: Chromium/Electron detection and flag injection ────────────
        // Extract the Windows executable name from the wine explorer command
        String effectiveExecutable = guestExecutable;
        if (guestExecutable != null) {
            // Parse out the actual EXE being launched (after 'wine' command)
            String[] parts = ProcessHelper.splitCommand(guestExecutable);
            String execName = parts.length > 0 ? parts[parts.length - 1] : "";
            if (ChromiumCompatHelper.isChromiumBased(execName)) {
                // Apply Chromium flags; also propagates ANGLE and GPU env vars
                effectiveExecutable = ChromiumCompatHelper.applyChromiumFlags(guestExecutable, envVars);
            }
        }

        String command = rootDir+"/usr/local/bin/box64 "+effectiveExecutable;
