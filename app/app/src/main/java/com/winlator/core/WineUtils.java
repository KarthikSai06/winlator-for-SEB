...
public abstract class WineUtils {
    ...
    public static void applySystemTweaks(Context context, WineInfo wineInfo) {
        ...
        try (WineRegistryEditor registryEditor = new WineRegistryEditor(userRegFile)) {
            ...
            // ── Phase 3: Modern Windows API compatibility DLL overrides ────────────
            // TLS / Networking — use Wine's native implementations for correct TLS 1.3
            // negotiation on Android (avoids bcrypt/schannel ARM issues).
            final String[] tlsLibs = {"winhttp", "wininet", "schannel"};
            for (String name : tlsLibs) {
                registryEditor.setStringValue(dllOverridesKey, name, "native,builtin");
            }

            // Cryptographic providers — bcrypt/ncrypt are required by .NET 6+ and
            // by SEB's key-generation code. Use Wine's builtin first.
            final String[] cryptoLibs = {"bcrypt", "ncrypt", "crypt32", "cryptsp", "cryptnet"};
            for (String name : cryptoLibs) {
                registryEditor.setStringValue(dllOverridesKey, name, "builtin,native");
            }

            // Media Foundation — required by Electron media playback and SEB's
            // WebRTC engine. Use native copies when available (from wincomponents).
            final String[] mfLibs = {"mf", "mfplat", "mfreadwrite", "mfplay", "msvproc"};
            for (String name : mfLibs) {
                registryEditor.setStringValue(dllOverridesKey, name, "native,builtin");
            }

            // COM automation — required by SEB's COM-based integrity checks
            final String[] comLibs = {"oleaut32", "ole32", "combase"};
            for (String name : comLibs) {
                registryEditor.setStringValue(dllOverridesKey, name, "builtin,native");
            }
