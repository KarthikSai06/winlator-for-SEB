...
public class WineRegistryEditor implements Closeable {
    ...
    /**
     * Returns true if the given key exists in the registry AND contains the specified value name.
     *
     * @param key  Registry key path (e.g. "Software\\Microsoft\\Cryptography").
     * @param name Value name to check.
     * @return true if both the key and value exist.
     */
    public boolean hasValue(String key, String name) {
        Location keyLocation = getKeyLocation(key);
        if (keyLocation == null) return false;
        return getValueLocation(keyLocation, name) != null;
    }
