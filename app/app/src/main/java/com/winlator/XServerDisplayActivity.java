...
import com.winlator.compat.DependencyInstaller;
import com.winlator.compat.EnvironmentSpoofer;
import com.winlator.compat.SEBCompatibilityLayer;
...
public class XServerDisplayActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    /** Live crash-analysis report populated by the streaming log callback. */
    private final CrashAnalyzer.CrashReport crashReport = new CrashAnalyzer.CrashReport();
    /** Whether SEB compatibility mode is active for the current session. */
    private boolean sebCompatMode = false;
    ...
    private void setupXEnvironment() {
        ...
        // Phase 5: Persistent shader cache on Android internal storage
        File shaderCacheDir = new File(getFilesDir(), "mesa_shader_cache");
        if (!shaderCacheDir.isDirectory()) shaderCacheDir.mkdirs();
        envVars.put("MESA_SHADER_CACHE_DIR",           shaderCacheDir.getAbsolutePath());
        envVars.put("MESA_SHADER_CACHE_DISABLE",       "false");
        envVars.put("MESA_SHADER_CACHE_MAX_SIZE",      "512MB");
        envVars.put("RADV_PERFTEST",                   "gpl");  // Shader caching pipeline
        ...
        // ── Phase 3/4: Compatibility layers ───────────────────────────────────
        // Install runtime dependencies (VCRedist 2022, .NET 4.8, WebView2 stub)
        DependencyInstaller.installAll(this, rootFS, false);

        // Detect SEB and apply its specific compatibility patches
        if (container != null) {
            String execPath = shortcut != null ? shortcut.path : getIntent().getStringExtra("exec_path");
            if (execPath != null && SEBCompatibilityLayer.isSEB(execPath)) {
                sebCompatMode = true;
                SEBCompatibilityLayer.apply(this, rootFS, envVars);
            }
        }

        // Apply environment spoofing to mask emulator fingerprints
        EnvironmentSpoofer.apply(this, rootFS, envVars);
    ...
    private void applyGeneralPatches(Container container) {
        ...
        // Phase 3: Force reinstall compat packages after a full patch cycle
        DependencyInstaller.installAll(this, rootFS, true);
    }
